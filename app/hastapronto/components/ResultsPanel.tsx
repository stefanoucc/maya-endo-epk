...
export function ResultsPanel({ isOpen, onClose, score, totalQuestions, correctAnswers }: ResultsPanelProps) {
  ...
  // Extract correctly guessed track numbers from correctAnswers
  const getCorrectlyGuessedTracks = () => {
    const correctTracks = new Set<number>();
    
    // Parse correctAnswers to extract object names and map them to track numbers
    correctAnswers.forEach(answer => {
      const objectName = answer.split(' - ')[0];
      // Map object names to track numbers (based on game logic)
      const objectToTrackMap: { [key: string]: number } = {
        'Cámara polaroid': 1,
        'Corazón de lana': 2,
        'Cartel de interrogatorio': 3,
        'Espejo roto': 4,
        'Zapatitos y reloj': 5,
        'Tazas de té': 6,
        'Leña': 7,
        'Álbumes de fotos': 8,
        'Pala y guantes': 9,
        'Medias de colores': 10,
        'Maleta amarilla': 11
      };
      
      if (objectToTrackMap[objectName]) {
        correctTracks.add(objectToTrackMap[objectName]);
      }
    });
    
    // Always show track 12 (bonus track)
    correctTracks.add(12);
    
    return correctTracks;
  };
  
  const correctlyGuessedTracks = getCorrectlyGuessedTracks();
  ...
  const albumTracklist = [
    ...
    { track: 11, title: "Hasta Pronto", emoji: "🧳💛" },
          { track: 12, title: "Pequeño mundo entre 2", emoji: "🌍🐱" }
  ];
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className={`bg-gradient-to-r px-3 py-2 border-b border-gray-400 flex items-center justify-between ${
          ...
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-white text-sm font-semibold">
              {isPerfectScore ? "tracklist desbloqueado" : "resultados del juego"}
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 bg-gray-100 max-h-96 overflow-y-auto">
          
          {/* Celebración solo para puntaje perfecto */}
          {isPerfectScore && (
            <div className="bg-gradient-to-r from-yellow-100 to-yellow-200 border-2 border-yellow-400 p-4 shadow-inner text-center mb-4"
                 style={{
          ...
          {/* Puntuación principal para resultados no perfectos */}
          {!isPerfectScore && (
            <div className="bg-white border-2 border-gray-400 p-4 shadow-inner text-center mb-4"
                 style={{
          ...
          {/* Tracklist del álbum - siempre visible */}
          <div className="bg-white border-2 border-gray-400 p-4 shadow-inner mb-4"
               style={{
                 borderTopColor: '#808080',
                 borderLeftColor: '#808080',
                 borderRightColor: '#fff',
            ...
            <div className="space-y-1.5">
              {albumTracklist.map((song) => {
                const isBoldTrack = ['Vino en Taza', 'Hasta Pronto', 'Mala Suerte', 'Pequeño mundo entre 2'].includes(song.title);
                const isUnlocked = correctlyGuessedTracks.has(song.track);
                
                return (
                  <div key={song.track} 
                       className={`flex items-center justify-between p-2 bg-gray-50 border border-gray-300 transition-colors ${
                         isUnlocked ? 'hover:bg-gray-100' : 'blur-sm opacity-60'
                       }`}
                       style={{
                         borderTopColor: '#fff',
                         borderLeftColor: '#fff',
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className={`text-sm text-gray-800 ${isBoldTrack ? 'font-bold' : 'font-medium'}`}>
                        {isUnlocked ? (
                          <>
                            <div>{song.title}</div>
                            {song.track === 12 && (
                              <div className="text-xs text-gray-600 font-normal italic">(Bonus Track)</div>
                            )}
                          </>
                        ) : (
                          '???'
                    ...
                    <span className="text-lg">{isUnlocked ? song.emoji : '🔒'}</span>
                  </div>
                );
              })}
            </div>
          ...
          {/* Respuestas correctas para resultados no perfectos */}
          {!isPerfectScore && correctAnswers.length > 0 && (
            <div className="bg-green-50 border-2 border-green-400 p-3 shadow-inner mb-4"
                 style={{
